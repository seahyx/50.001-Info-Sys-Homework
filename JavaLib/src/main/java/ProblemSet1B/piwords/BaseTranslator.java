...
public class BaseTranslator {
    ...
    public static int[] convertBase(int[] digits, int baseA,
        ...
        for (int j: digits) if (j < 0 || j >= baseA) return null;

        int[] digitsBaseA = digits.clone();

        // Digits array in new base B
        int[] digitsBaseB = new int[precisionB];

        for (int i = 0; i < precisionB; i++) {
            int carry = 0;
            for (int j = digitsBaseA.length - 1; j >= 0; j--) {
                int x = digitsBaseA[j] * baseB + carry;
                digitsBaseA[j] = x % baseA;
                carry = x / baseA;
            }
            digitsBaseB[i] = carry;
        }
