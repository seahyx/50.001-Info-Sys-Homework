...
public class AlphabetGenerator {
    ...
    public static char[] generateFrequencyAlphabet(int base,
                                                   String[] trainingData) {
        // Count number of characters each
        ...
        // Convert char keyset into a treeset, allowing for sorted iteration
        TreeSet<Character> keyTS = new TreeSet<>(charCountMap.keySet());

        // Iterate through each character and correspond them to the output character map.
        char[] outputMap = new char[base];
        int lastIdx = 0;
        double cdf = 0;
        for (char ch: keyTS) {
            cdf += (double) charCountMap.get(ch) / totalChars;
            double cdfScaled = cdf * base;
            for (int i = lastIdx; cdfScaled > i; i++) {
                outputMap[i] = ch;
                lastIdx = i + 1;
            }
        }
